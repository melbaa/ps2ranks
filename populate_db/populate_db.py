...
class DeleteInactive:
  def __init__(self, ioloop, logger, conn_ctx):
    ...
    self.max_lastseen_qry = "select max(lastseen) from chartbl"

    self.old_qry_predicate = " where ({}-lastseen)/3600/24 > (br*0.2+11)"

    self.count_old_qry = \
      """
        select count(*) from chartbl
        {}
    ...
    self.select_old_qry = \
      """
        select name, br, lastseen/3600/24, ({}-lastseen)/3600/24, '>' , br*0.2+11
        from chartbl
        {}
        limit 20
      """.format("{}" ,self.old_qry_predicate)
  ...
  def delete_tick(self):
    try:
      ...
      self.conn_ctx.cursor.execute(self.count_old_qry.format(max_lastseen))
      ...
      log_msg = "deleted {} rows gg".format(self.conn_ctx.cursor.rowcount);
      self.logger.debug(log_msg)
...
class SummaryTableUpdate:
  def __init__(self,ioloop,logger,conn_ctx):
    ...
    self.find_avg_rating_qry = \
      """
        select avg(rating) from chartbl
      """
  ...
  def avg_player_rating_tick(self):
    ...
    try:
      ...
      self.conn_ctx.cursor.execute(self.insert_update_qry, (str(avg), str(avg)))
...
class Populate_db:
  def __init__(self,ioloop, serviceid, useragent, logger, conn_ctx):
    ...
    #default_volatility (sigma) 0.06 and tau (r constraint on volatility) 0.5 RD ~ 65
    ...
    self.insert_qry = 'insert into chartbl (rating,rd,volatility,lastseen,lastopponent,name,world,faction,br) values \
    ( %s, %s, %s, %s, %s, %s, %s, %s, %s)'
    self.select_qry = "select rating, rd,volatility from chartbl where name =%s"
    self.update_qry = "update chartbl set rating=%s, rd=%s, volatility=%s, lastseen=%s, world=%s, faction=%s, br=%s where name=%s"
  ...
  def select_update_tick(self):
    ...
    try:
      ...
      self.timestamp,num_events = self.update_glicko_ratings(json_reply,self.timestamp)
      ...
      self.conn_ctx.cursor.execute(self.last_update_qry, (curr_time, curr_time))
    ...
    self.ioloop.add_timeout(datetime.timedelta(seconds=self.time_to_sleep), cb)
  ...
  def find_nick_or_default(self, nick):
    ...
    if result is None:
      #insert defaults
      try:
        self.conn_ctx.cursor.execute(self.insert_qry, (self.default_rating, self.default_rd, self.default_volatility, 0, '', nick,'','',0))
      ...
      rating,rd,sigma = float(self.default_rating),float(self.default_rd),float(self.default_volatility)
    else:
      rating,rd,sigma = float(result[0]), float(result[1]), float(result[2])
  ...
  def update_glicko_ratings(self,json_reply,old_timestamp):
    ...
    try:
      if 'errorCode' in json_reply:
        self.logger.error('soe returned error {}'.format(str(json_reply)))

      elif len(json_reply['event_list']) == 0:
        self.logger.info('no events... servers down? timestamp: {}'.format(timestamp))
      else:
        """
        ...
        #logger.debug(json_reply['event_list'][-1]['timestamp'])  #from (smaller, earlier)

        #we'll reset to 0 and find the time of the last kill
        #need the timestamp for after= argument when we request killboard
        timestamp = 0
        must_commit = False
        for event in reversed(json_reply['event_list']):
          ...
          try:
            ...
            #even theres only an attacker or loser
            ...
            attacker_br = int(event['attacker']['battle_rank']['value'])
            attacker_faction = event['attacker']['faction']['code_tag']

            loser = event['character']['name']['first']
            loser_br = int(event['character']['battle_rank']['value'])
            ...
            if attacker != loser:
              ...
              #find nicks in the database or insert them with default values
              ...
              #glicko 1v1
              att_glicko_rat = self.glicko.create_rating(attacker_rating, attacker_rd,attacker_volatility)
              lose_glicko_rat = self.glicko.create_rating(loser_rating, loser_rd, loser_volatility)

              try:
                new_att, new_lose = self.glicko.rate_1vs1(att_glicko_rat, lose_glicko_rat)
                ...
                #rating
                assert 0 < new_att.mu < 3000, 'wtf assertion error for attacker rating'
                assert 0 < new_lose.mu < 3000, 'wtf assertion error for loser'

                #volatility
                assert 0.01 < new_att.sigma < 1.2, 'wtf assertion error for attacker volatility'
                assert 0.01 < new_lose.sigma < 1.2, 'wtf assertion error for loser volatility'

                must_commit = True
                self.conn_ctx.cursor.execute(self.update_qry, (new_att.mu, new_att.phi, new_att.sigma, newtimestamp, world, attacker_faction, attacker_br, attacker))
                #logger.info(cursor.statement)
                self.conn_ctx.cursor.execute(self.update_qry, (new_lose.mu, new_lose.phi, new_lose.sigma, newtimestamp, world, loser_faction, loser_br, loser))
                #self.logger.debug('updating with stmt ' + self.conn_ctx.cursor.statement)
              except OverflowError as e:
                self.logger.error(str(e) + ' attacker: {} {}; loser: {} {}; json: {}'.format(attacker, att_glicko_rat,loser, lose_glicko_rat,str(json_reply)))
            ...
            for err in e.args:
              ...
              if err not in {'attacker', 'character', 'world'}:
                self.logger.error('key not found {}; event is {}'.format(err, event))
            #logger.error(event)
